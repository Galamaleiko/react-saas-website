                ...
                <div className='testimonials_inner-before testimonials_inner-after relative -my-12 -mr-3 flex items-start max-lg:static max-md:block'>
                    <div className='testimonials_group-after flex-50'>
                        {testimonials.slice(0, halfLength).map((item) => (
                            <TestimonialItem item={item} containerClassName='last:after:hidden last:after:max-md:block' />
                        ))}
                    </div>

                    <div className='flex-50'>
                        {testimonials.slice(halfLength).map((item) => (
                            <TestimonialItem item={item} containerClassName='last:after:hidden after:right-auto after:left-0 after:max-md:-left-4 md:px-12' />
                        ))}
