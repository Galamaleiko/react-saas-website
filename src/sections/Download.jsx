        ...
        <section>
            <Element name='download' className='g7 relative pb-32 pt-24 max-lg:pb-24 max-md:py-16'>
                <div className='container'>
                                ...
                                {links.map(({ id, title, icon, url }) => (
                                    <li key={id} className='download_tech-link download_tech-link_last-before download_tech-link_last-after'>
                                        <a href={url} className='size-22 download_tech-icon_before relative flex items-center justify-center rounded-half border-2 border-s3 bg-s1 transition-borderColor duration-500'>
                                            <span className='-top-2 absolute rotate-90'>
                                                <Marker />
                                            </span>
                    ...
                    <ul className='flex gap-6 justify-around mt-24 max-lg:hidden'>
                        {logos.map((logo) => (
                            <li key={logo.id}>
                                <img src={logo.url} width={logo.width} height={logo.height} alt={logo.title} />
                            </li>
                        ))}
                    </ul>
                </div>
